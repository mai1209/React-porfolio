const PorfolioInfo = [
    {
        id: 1,
        work: 'Portfolio made with Js',
        link: 'Ingresa a la web del proyecto',
        URl: 'https://https://maira-cv.netlify.app/',
        img: require("../img/image6.jpg"),
        alt: "imagen1"
        
        
    },
    {
        id: 2,
        work: 'Blog made with Sass',
        link: 'Ingresa a la web del proyecto.',
        URl: 'https://blog-con-sass.netlify.app/',
        img: require("../img/image2.jpg")

    },
   
    {
        id: 3,
        work: 'Beauty center page made with Js',
        link: 'Ingresar a la web del proyecto',
        URl: 'https://unique-page.vercel.app/',
        img:  require("../img/image3.jpg")
    },
    {
        id: 4,
        work: 'TO DO LIST made with Js',
        link: 'Ingresar a la web del proyecto',
        URl: 'https://to-do-list-green-six.vercel.app',
        img:  require("../img/image4.jpg")
    },
    {
        id: 5,
        work: 'Page made only with HTML',
        link: 'Ingresar a la web del proyecto',
        URl: 'https://trabajo4-nucba.vercel.app',
        img:  require("../img/image5.jpg")
    },
    {
        id: 6,
        work: 'Login made with Js',
        link: 'Ingresar a la web del proyecto',
        URl: 'https://sesion-ten.vercel.app/',
        img:  require("../img/image5.jpg")
    },
 
]


export default PorfolioInfo;